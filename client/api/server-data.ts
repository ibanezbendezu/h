...
export const fileDataRequest = async (id: number, clusterId: number) => {
    try {
        const res = await axios.get(`/files/${id}/${clusterId}`);
        return {data: res.data};
    } catch (error) {
...
export const fileDataRequestBySha = async (sha: string, clusterId: number) => {
    try {
        const res = await axios.get(`/files/sha/${sha}/${clusterId}`);
        return {data: res.data};
    } catch (error) {
        console.error("Error fetching file data:", error);
        throw error;
