...
import {fileContentRequest, fileContentRequestBySha, pairByClusterDataRequest, pairByClusterShaDataRequest} from "@/api/server-data";
import {useAuthStore} from "@/store/auth";
import {Box, User, ChevronDown, ChevronUp, Code} from "lucide-react";
...
export default function FilePage({params}: { params: any }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            const res = await pairByClusterShaDataRequest(params.id, params.fileId);
        ...
        <div className="m-10">
            ...
            <div className="my-4 flex items-center gap-2">
                <p className="text-sm font-normal text-muted-foreground">
                    Revisa las coincidecias de este archivo con otros archivos en un comparación del código fuente lado a lado.
                </p>
            </div>
                        ...
                        {data?.repositories.map((repository: any, index: number) => (
                            <React.Fragment key={index}>
                                <SelectGroup>
                                    ...
                                    {repository.pairs.map((pair: any, pairIndex: number) => (
            ...
            <div className="grid grid-cols-2 gap-2">
                <CodeViewer code={file?.code ?? ""} language={file?.file.language} highlightRange={fileFragments} color={"#4d4d4d"}/>
                {pairFile ? (
                    <CodeViewer code={pairFile.code} language={pairFile.file.language} highlightRange={pairFile.file.fragments} color="#4d4d4d" />
                ) : (
                    <CodeViewer.Void />
                )}
            </div>
