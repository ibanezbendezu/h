...
import {pairByClusterDataRequest, pairByClusterShaDataRequest, pairByIdDataRequest} from '@/api/server-data';
import {fileCytoscape} from './data';
...
export const Cluster: React.FC<Props> = ({data, clusterId}) => {
    ...
    const handleFile = async (e: any) => {
        const fileId = parseInt(e.id.split("-")[1], 10);
        const fileSha = e.sha;
    ...
    return (
        <>
            ...
            <CytoscapeComponent
                cy = {(cy) => {
                    setCy(cy);
                    cy.on("click", "edge", (e: any) => { handlePair(e.target.data()); } );
                    cy.on("click", "node[type='file']", (e: any) => { handleFile(e.target.data()); } );
                }}
